...
void * hostapd_ctrl_iface_pmksa_create_entry(struct hostapd_data *hapd, char *cmd)
{
	u8 spa[ETH_ALEN];
	u8 pmkid[PMKID_LEN];
	u8 pmk[PMK_LEN_MAX];
	...
	return wpa_auth_pmksa_create_entry(hapd->wpa_auth, hapd->own_addr, spa, pmk, pmkid, expiration);
}
